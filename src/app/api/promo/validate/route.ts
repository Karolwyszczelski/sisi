...
export async function POST(req: Request) {
  ...
  // 3) Globalny limit użyć (jeśli ustawiony)
  ...
  // 4) Jednorazowość na użytkownika/konto (po emailu lub userId, z pominięciem anulowanych/nieudanych)
  if (email || userId) {
    const orParts: string[] = [];
    if (email) orParts.push(`contact_email.ilike.${email}`);
    if (userId) orParts.push(`user.eq.${userId}`);
    ...
    const { count, error: usedErr } = orParts.length ? await q.or(orParts.join(",")) : await q;
    if (!usedErr && (count || 0) > 0) {
      return NextResponse.json(
        { valid: false, message: "Kod został już użyty na Twoim koncie." },
        { status: 400 }
      );
    }
  }

  // 5) Zwróć parametry zniżki
